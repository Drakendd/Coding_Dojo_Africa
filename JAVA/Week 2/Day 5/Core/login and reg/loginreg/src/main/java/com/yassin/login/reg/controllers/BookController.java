...
public class BookController {
	...
	//display Edit Form
	 ...
	 public String updateBook(@PathVariable("id")Long id, Model model, HttpSession  session) {
		...
		Book book = bookService.findBookByID(userId);
		 model.addAttribute("book",book);
		 return "edit.jsp";
	 }
	 // action route for edit 
	 @PutMapping("books/{id}")
	 public String editBook(@Valid @ModelAttribute("car") Book book ,BindingResult result ,  HttpSession session) {
		Long userId = (Long) session.getAttribute("user_id") ; 
		 if (result.hasErrors()) {
				return "edit.jsp";
			}else {
